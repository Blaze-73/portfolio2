...
function getEffectiveSpeed(value: string, reducedMotion: boolean) {
  const min = 0;
  const max = 100;
  ...
  const initPixels = () => {
    ...
    for (let x = 0; x < width; x += gapPx) {
      for (let y = 0; y < height; y += gapPx) {
        ...
        pxs.push(new Pixel(canvasRef.current, ctx!, x, y, color, getEffectiveSpeed(String(finalSpeed), reducedMotion), delay));
      }
    }
